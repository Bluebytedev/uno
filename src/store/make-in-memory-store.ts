...
export default (config: BaileysInMemoryStoreConfig) => {
	...
	const mergeDuplicateContacts = (contactId: string) => {
		// Verifica se é um contato com lid: (Local ID)
		if (contactId.includes('lid:')) {
			// Procura por um contato com o mesmo número mas com JID normal (@s.whatsapp.net)
			const phoneNumber = contactId.split(':')[1]?.split('@')[0]
			if (phoneNumber) {
				const normalJid = `${phoneNumber}@s.whatsapp.net`
				
				// Se existe um contato com JID normal, mescla os dados
				if (contacts[normalJid]) {
					logger.debug({ lidContact: contactId, normalContact: normalJid }, 'Merging duplicate contact with lid')
					
					// Mescla os dados do contato lid: com o contato normal
					contacts[normalJid] = Object.assign(
						{},
						contacts[contactId] || {},
						contacts[normalJid]
					)
		...
		// Verifica o caso inverso: se é um JID normal e existe um lid: duplicado
		if (contactId.includes('@s.whatsapp.net')) {
			const phoneNumber = contactId.split('@')[0]
			const lidJid = `lid:${phoneNumber}@lid`
			
			if (contacts[lidJid]) {
				logger.debug({ normalContact: contactId, lidContact: lidJid }, 'Merging duplicate contact with normal JID')
				
				// Mescla os dados do contato lid: com o contato normal
				contacts[contactId] = Object.assign(
					{},
					contacts[lidJid],
					contacts[contactId] || {}
				)
	...
	const contactsUpsert = (newContacts: Contact[]) => {
		...
		for(const contact of newContacts) {
			...
			// Mescla contatos duplicados antes de fazer upsert
			const mergedContactId = mergeDuplicateContacts(contact.id)
			
			contacts[mergedContactId] = Object.assign(
				contacts[mergedContactId] || {},
				contact
			)
		}
